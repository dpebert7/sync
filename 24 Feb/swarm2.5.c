...
// gcc swarm2.5.c -o temp -lglut -lm -lGLU -lGL
// ./temp
...
#define K		N-1	// How many neighbors to look at. Maximum is N-1
	...
	while(time < STOP_TIME)
	{
		...
		{	
			...
			{	
				...
				vn[i][j] = 0.0;
				for(k=0; k<K; k++) // K nearest neighbors
				{
					vn[i][j] += (p[MIN_POS[i][k]][j]-p[i][j])/(r[MIN_POS[i][j]][k]*r[MIN_POS[i][j]][k]+EPSILON);
			...
			// Find magnitude
			magc = sqrt(vc[i][0]*vc[i][0] + vc[i][1]*vc[i][1] + vc[i][2]*vc[i][2] + EPSILON); // THESE EPSILONS MATTER!!!
		...
		//*
			...
			for(j=0; j<3; j++)
			{
				vc[i][j]*=(5.0); 			// magnitude of center part
				vn[i][j]*=(r[i][0] + EPSILON);		// magnitude of neighbor part
