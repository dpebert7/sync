...
// gcc swarm2.1.c -o temp -lglut -lm -lGLU -lGL
// ./temp
...
#define SEARCH_DISTANCE	0.5	//Only care about neighbors that are within this radius 

#define N 7  // number of bodies
...
/*
	...
	for(i = 0; i<N; i++){
		...
		p[i][1] = cos(i*2*PI/N);
		p[i][2] = 0.0;
		for(j=0; j<3; j++){  // j<3, not j<N !!!!???? WHAAATT?? WHY IS THIS MESSING CRAP UP??!!!
			v[i][j] = 0.0;
		}
	}
//*/
...
int n_body()
{
	...
	while(time < STOP_TIME)
	{
		...
		{	
			...
			{	
				...
				for(k=0; k<N; k++)
				{
					...
					{
						vn[i][j] -= (p[k][j]-p[i][j])/(r[i][k]+EPSILON);
					}
				}
			}
		...
		//*
			for(j=0; j<3; j++)
			{
				vc[i][j]*=20.0; 		// magnitude of center part
				vn[i][j]/=(0.5*MINIMUM[i][0]);	// magnitude of neighbor part
			...
			//printf(" The velocity components of particle %d are %.2f, %.2f, %.2f\n.", i, v[i][0],v[i][1],v[i][2]);
		//*/
		}
		//printf("The x componetns of vc, vn, and v for the first particle are: %.5f, %.5f, %.5f\n", vc[i][0], vn[i][0], v[i][0]);
