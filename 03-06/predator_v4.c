...
void mouseFunc( int button, int state, int x, int y )
{
	...
	if( button == GLUT_LEFT_BUTTON ) 
	{
		...
		{
			printf("MOUSEPOINTER is %i \n", MOUSEPOINTER);
			coord[0] = (x*4.0/XWindowSize)-2.0;
			coord[1] = -(y*4.0/YWindowSize)+2.0;
			coord[2] = 0.0;
			printf("The food is at (%.4f, %.4f, %.4f)\n",
				coord[0], coord[1], coord[2]);
			particle[MOUSEPOINTER].p[0] = coord[0];
			particle[MOUSEPOINTER].p[1] = coord[1];
			particle[MOUSEPOINTER].p[2] = coord[2];
			
			// Change pointer to next food particle
			MOUSEPOINTER++;
			if(MOUSEPOINTER == NFISH+NFOOD)
			{
				MOUSEPOINTER = NFISH;
			}			
		}
