...
//nvcc fixit_v1.cu -o temp -lglut -lm -lGLU -lGL && ./temp
...
#define WA			1.0 // Attraction Weight Ratio
#define WD 			1.5 // Directional Weight Ratio
#define CA 			4.0 // Attraction Coefficient
#define CR			256.0 // Repulsion Coefficient
#define CPR			10.0 // Repulsion Coefficient (predator)
#define CTA			10.0 // Attraction Coefficient (Target)
#define BOUNDARY		EYEZ-1.0 // Walls
#define BDIST			10.0 // Distance from boundary at which turning should start.

// Constants for Fishies
#define N			256 // Number of fish
...
float3 getWallForces(float4 p0, int dim)
{
	...
	if(p0.x > (30.0))
	{
		f.x -= (30.0-p0.x)*(30.0-p0.x);
	}
		
	// Left Wall
	if(p0.x < (-30.0))
	{
		f.x += (p0.x-30.0)*(p0.x-30.0);
	}
		
	// Top Wall
	if(p0.y > (30.0))
	{
		f.y -= (30.0-p0.y)*(30.0-p0.y);
	}
		
	// Bottom Wall
	if(p0.y < (-30.0))
	{
		f.y += (p0.y-30.0)*(p0.y-30.0);
	}
...
void getForces(float4 *p, float3 *v, float3 *f, float4 *p_pred, float4 *p_targ)
{
	...
	for(i=0; i<N; i++)
	{
		...
		*/
		...
		if(forceNormalizer>100.0)
		{
			f[i].x /= forceNormalizer/300.0;
			f[i].y /= forceNormalizer/300.0;
			f[i].z /= forceNormalizer/300.0;
		}
