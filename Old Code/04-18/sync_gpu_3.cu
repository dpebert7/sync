...
//Compiling: nvcc sync_gpu_3.cu -o sync_3 -lglut -lm -lGLU -lGL && ./sync_3
...
#define STOP_TIME		100000.0 	// How long to go
...
__device__ int getRadiusNumber(float4 p0, float4 p1)
{
	int fishCounter = 0;
	float dx = p1.x - p0.x;
	float dy = p1.y - p0.y;
...
__device__ float3 getFishForces(float4 p0, float4 p1, float3 v1, int fishCounter)
{
	float3 f;
	float dx = p1.x - p0.x;
	float dy = p1.y - p0.y;
...
__global__ void getForces(float4 *pos, float3 *vel, float3 *force, float4 *predp, int m, int p, float4 *predtarg, float speed)
{
	...
	for(j=0; j<gridDim.x; j++)
	{
		...
		for(i=0; i<blockDim.x; i++)
		{
			...
			if(ii != id)
			{	
				numberFishies = getRadiusNumber(posMe, shPos[i]);
				forceMag = getFishForces(posMe, shPos[i], shVel[i], numberFishies);
